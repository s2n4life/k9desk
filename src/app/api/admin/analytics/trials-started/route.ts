...
/**
 * GET /api/admin/analytics/trials-started
 * 
 * Returns count of trials that started in the selected timeframe.
 */
export async function GET(request: Request) {
    try {
        ...
        // Count trials created in this period
        let query = supabase
            .from('businesses')
        ...
        const { count: trialsStarted } = await query;

        const result: any = {
            value: trialsStarted || 0
        };
        ...
        if (compare) {
            ...
            const { count: prevTrialsStarted } = await prevQuery;
            const { delta, deltaPercent } = calculateDelta(trialsStarted || 0, prevTrialsStarted || 0);

            result.previousValue = prevTrialsStarted || 0;
            result.delta = delta;
        ...
        console.error('[Analytics Trials Started] Error:', error);
        return NextResponse.json(
            { error: 'Failed to fetch trials started', details: error.message },
            { status: 500 }
        );
    }
}
